...
public class userS : MonoBehaviour {
	...
	void Start () {
		tms = GameObject.FindGameObjectWithTag(tileManagerTag).GetComponent<tileManagerS>();
	}
	...
	void Update() {
		...
		if(Input.GetKey(KeyCode.LeftControl) && Input.GetKey(KeyCode.LeftShift) && Input.GetMouseButton(0))
		{
			...
			{
				hex_script          = clicked_game_object.GetComponent<hexS>();
				userS.tms.CreateHex(true, 1,
											userS.tms.border_hex, clicked_game_object.transform.position,  
											tileManagerS.Tiles.EditorTileA, hex_script.x_coord, hex_script.z_coord);
			}
		}
		else
		//shift + left click
		if(Input.GetKey(KeyCode.LeftShift) && Input.GetMouseButton(0))
		{
			...
			{
				hex_script          = clicked_game_object.GetComponent<hexS>();
				userS.tms.CreateHex(true, userS.brush_size, 
											userS.last_created_hex_go, clicked_game_object.transform.position,  
											userS.last_created_hex_type, hex_script.x_coord, hex_script.z_coord);		
			}
		}
		else
		//alt + left click
		if(Input.GetKey(KeyCode.LeftAlt) && Input.GetMouseButtonDown(0))
		{
			...
			{
				hex_script          				= clicked_game_object.GetComponent<hexS>();
				userS.last_created_hex_type 	= hex_script.tile_type;
				userS.last_created_hex_go   	= tileManagerS.hex_dict[hex_script.tile_type];
			}
		}
		else
		//ctrl + left click
		...
		{ 
//			userS.selected_hex = this.gameObject;
//			print (userS.selected_hex);
...
//				hexS hex_tile = hit.transform.gameObject.GetComponent<hexS>();
...
//					hexS hex_tile = hex.GetComponent<hexS>();
//					hex_tile.CloneNorth();
