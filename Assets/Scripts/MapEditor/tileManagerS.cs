...
//		tile_db  
		...
		CreateHex(true, 1, initial_hex, new Vector3(0, 0, 0), initial_hex.GetComponent<hexS>().tile_type, 0, 0);
		//GameObject origin_hex = 
		//hexS origin_hex_script = origin_hex.GetComponent<hexS>();
		
		//origin_hex_script.SurroundHex(border_hex, border_hex.GetComponent<hexS>().tile_type); //-1 denotes tempborders
...
//	
//	public void CloneHex(GameObject hex_type, tileManagerS.Tiles data_type, float x_dist, float z_dist, int x_coord_in, int z_coord_in)
//	{ 
...
//	}
//	
//	public void BuildHex(GameObject go, tileManagerS.Tiles data_type,  Vector3 pos, int x_coord_in, int z_coord_in)
//	{
...
//			hexS new_hex_script = new_hex.GetComponent<hexS>();
//			
...
//	}
	...
	private GameObject InstantiateHex(GameObject hex_type, Vector3 pos, tileManagerS.Tiles data_type, int x, int z)
	{
		
		GameObject new_hex = (GameObject) Instantiate(hex_type, pos, Quaternion.identity);
		hexS new_hex_script = new_hex.GetComponent<hexS>();
		
//		new_hex_script.tile_num = ++userS.tile_counter;
...
//		}
	...
	public GameObject CreateHex(bool overwrite, int depth, GameObject hex_type, Vector3 pos, tileManagerS.Tiles data_type, int x, int z)
	{
		depth--;
		
		if(data_type != Tiles.EditorTileA)
		{
			userS.last_created_hex_type = data_type;
			userS.last_created_hex_go   = hex_type;
		}
		
		GameObject created_hex;
		hexS created_hex_script;
		print ("creating : " + x + ", " + z);
		if(tile_db.ContainsKey(x))
		{ 
			if(tile_db[x].ContainsKey(z))
			{
				if((overwrite && tile_db[x][z].getDataType() != data_type) || (tile_db[x][z].getDataType() == Tiles.EditorTileA && data_type != Tiles.EditorTileA))  //tile_db[x][z].getDataType() == Tiles.EditorTileA && data_type != Tiles.EditorTileA
				{
					...
					if(tile_db[x][z].getOccupier() != userS.last_created_hex_go)
					{
						created_hex = InstantiateHex(hex_type, pos, data_type, x, z);
						created_hex_script = created_hex.GetComponent<hexS>();
...
//			}
	...
	public void SurroundHex(GameObject hex_type, Vector3 center_pos, tileManagerS.Tiles data_type, int depth, int x_coord, int z_coord)
	{
		bool overwrite_enabled;
		if(data_type != tileManagerS.Tiles.EditorTileA && userS.overwrite_mode)
		{
		...
		userS.tms.CreateHex(overwrite_enabled,  depth, hex_type, new Vector3(x_trans, 0, z_trans), data_type, x_coord, z_coord + 1);
		
		//CloneNorthEast
		x_trans = 2.30024F 	  + center_pos.x;
		z_trans = 1.3280592F  + center_pos.z;
		userS.tms.CreateHex(overwrite_enabled,  depth, hex_type, new Vector3(x_trans, 0, z_trans), data_type, x_coord + 1, z_coord);
		
		//CloneSouthEast
		x_trans = 3.14219F    + center_pos.x;
		z_trans = -0.486092F  + center_pos.z; 
		userS.tms.CreateHex(overwrite_enabled,  depth, hex_type, new Vector3(x_trans, 0, z_trans), data_type, x_coord + 1, z_coord - 1);
	...
	private class TileData {
		...
		public int x_coord;
		public int z_coord;
		public tileManagerS.Tiles data_type;
		...
		public tileManagerS.Tiles getDataType()
		{
			return data_type;
		}
